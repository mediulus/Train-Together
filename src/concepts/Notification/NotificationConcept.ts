...
export default class NotificationsConcept {
   ...
   */
  async addRecipient(
    { notificationId, user }: { notificationId: NotificationID; user: User },
  ): Promise<Empty | { error: string }> {
    ...
    // If the user is already a recipient, nothing to change.
    if (notification.recipients.includes(user)) {
      return {};
    }

    const updatedRecipients = [...notification.recipients, user];

    // Persist the updated recipients list to the DB.
    await this.notifications.updateOne(
      { _id: notificationId },
